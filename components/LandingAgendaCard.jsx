...
const LandingAgendaCard = ({item}) => {
  return (
    <div className='bg-white p-5 cursor-pointer mb-5'>
        <div className='flex justify-between items-start'>
            <div className='text-[#003756] font-bold text-lg leading-5'>
                <h1>{item.date.slice(0,4)}</h1>
                <h1>{item.date.slice(4,14)}</h1>
                <h1>{item.date.slice(14,18)}</h1>
            </div>
            <p>
                <Link href={item.link} className='text-[10px] font-semibold underline'>View more</Link>
            </p>
        </div>
        <div className='my-2'>
